...
TimeSlot::TimeSlot()
    : id(-1), day(""), startHour(0), startMinute(0), endHour(0), endMinute(0) {}

TimeSlot::TimeSlot(int id, const std::string &day, int startHour,
...
std::string TimeSlot::toString() const {
  ...
  oss << day << " " << std::setfill('0') << std::setw(2) << startHour << ":"
...
void ConstraintChecker::addProfessorAvailability(int professorId,
                                                 int timeslotId) {
  professorAvailability[professorId].insert(timeslotId);
}

void ConstraintChecker::addCoursePrerequisite(int courseId,
                                              int prerequisiteId) {
...
bool ConstraintChecker::isValidAssignment(
  ...
  if (!checkProfessorAvailability(assignment.professorId,
...
bool ConstraintChecker::checkTimeConflict(
    int professorId, int timeslotId,
...
bool ConstraintChecker::checkProfessorAvailability(int professorId,
...
bool ConstraintChecker::checkPrerequisites(
    int courseId, const std::vector<Assignment> &assignments) const {
