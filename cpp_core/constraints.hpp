...
private:
    ...
    std::unordered_map<int, std::unordered_set<int>> professorAvailability; // professor -> timeslot IDs
    std::unordered_map<int, std::unordered_set<int>> coursePrerequisites;   // course -> prerequisite course IDs
...
public:
    ...
    bool checkPrerequisites(int courseId, const std::vector<Assignment>& assignments) const;
    
    // Get available options for a course
    std::vector<int> getAvailableTimeslots(int courseId, int professorId,
                                          const std::vector<Assignment>& assignments) const;
    
    // Validation messages
    std::string getViolationMessage(const Assignment& assignment,
                                   const std::vector<Assignment>& assignments) const;
};
